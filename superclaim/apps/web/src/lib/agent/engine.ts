...
import { generateCollectionEmail } from './gemini'
import { sendCollectionEmail } from '@/lib/email/agentmail'
import { sendSms, buildCollectionSms } from '@/lib/sms/elks'
...
async function executeNode(
    ...
    switch (node.type) {
        ...
        case 'trigger': {
            // Pass through to next node immediately
            const next = getNextNode(flow, node.id)
            return { nextNodeId: next?.id || null, stepIncrement: false }
        }

        case 'delay': {
            // If we just arrived at this delay node, schedule the wait
            // The engine will pick it up again after next_action_at
            ...
            // On NEXT run (after delay elapsed), we advance past this node
            // Check if we're resuming from this delay (already sitting on it)
            if (claim.current_node_id === node.id) {
                const next = getNextNode(flow, node.id)
                return { nextNodeId: next?.id || null, stepIncrement: false }
            }

            // First time hitting this delay — stop and wait
        ...
        case 'sms': {
            ...
            if (claim.debtor_phone) {
                try {
                    ...
                    const smsMessage = buildCollectionSms({
                        debtorName: claim.debtor_name,
                        amount: claim.amount,
                        currency: claim.currency,
        ...
        case 'condition': {
            // Evaluate the condition — check if debtor has replied
            const replied = await hasDebtorReplied(claim.id)

            // "yes" handle = replied, "no" handle = not replied
        ...
        case 'end': {
            // Don't change status — just stop the flow
            await supabaseAdmin.from('claims').update({
                current_node_id: node.id,
                next_action_at: null,
                updated_at: new Date().toISOString(),
